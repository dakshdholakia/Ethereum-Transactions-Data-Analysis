...
TOKEN = "pnTVZrPfeDLu-ZR-XjB2S1gPbx-sBe9koruwIcAvJtmxQJky_h51NPqlAXM7BGCOMD2HMPG7kwQui4O6GiSPSw=="

client = InfluxDBClient(
    url=f"http://{host}:{port}", token=TOKEN, org="DJSCE")
...
def write_to_influxdb(df, epoch_id):
    ...
    for row in df.rdd.collect():
        ...
        point = Point("measurement1").tag("eth", "eth") \
...
print("WRITING BATCH COMPLETED !")

print("QUERY ##########################################")
query_api = client.query_api()

query = """from(bucket: "eth_block")
  |> range(start: -10m)
  |> filter(fn: (r) => r._measurement == "measurement1")
  |> mean()"""
tables = query_api.query(query, org="DJSCE")
